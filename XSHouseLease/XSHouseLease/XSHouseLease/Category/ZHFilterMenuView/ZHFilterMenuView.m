...
//        CGFloat time = self.showInWindow ? self.waitTime : 0.2f;
...
- (void)setEsModel:(XSHouseEsModel *)esModel dict:(NSDictionary *)dict{
    ...
    for (int i = 0; i < self.dataArr.count; i++) {
        // 区域数组[位置、附近]
        NSArray *modelArr = [self.dataArr safeObjectAtIndex:i];
        ZHFilterModel *filterModel = [modelArr firstObject];
        
        for (int y = 0; y < modelArr.count; y++) {
            //位置
            ZHFilterModel *model = [modelArr safeObjectAtIndex:y];
            if (model == filterModel) {
               model.selected = YES;
            }else{
               model.selected = NO;
            ...
            [model setModelItemSelectesNO];
            [model defSelectWithDict:dict];
//            if (i == 0 && y == 0) {
//                if (esModel.regionId && esModel.townId ) {
//                    [model setModelItemSelectesWithEsModel:esModel];
//                }else{
//                    if (model.selectFirst) {
//                        [[model.itemArr firstObject] setSelected:YES];
//                    }
...
//            }
            ...
            BOOL isSel = NO;
             for (ZHFilterItemModel *sub in model.itemArr) {
                 if (sub.selected) {
                     isSel = YES;
                     break;
                 }
        ...
        ZHFilterMenuDownType downType = [self getDownTypeBySelectedTabIndex:i];
        if (downType == ZHFilterMenuDownTypeTwoLists) {
            //根据二级列表是否有值来动态显示二级列表
            [self updateTableViewWidthWithItemArrHaveData:filterModel.itemArr.count];
        }
...
- (NSArray *)getSelectedTabIndexFilterModelArr222
{
    if (self.filterDataArr.count > self.selectedTabIndex) {
        return self.filterDataArr[self.selectedTabIndex];
    }
    NSLog(@"请传入正确的数据源");
    return @[];
}

...
// 2:配置====》有哪些使用方案
A = Build Setting
1.项目定义的多种编译环境（configurations），目前项目默认自带二个 Debug 与Release
    XCBuildConfiguration
    1.是什么：一种编译方案
    2.包含什么：键值对，都是对（硬件平台SKD/资源/资产目录编译选项/编译文件路径/编译选项/部署信息/链接/包/搜索路径/签名/语言/预编译/...）的设置
    3.怎么算进去的：由项目创建生成的
...
C = BuildRules
1.是什么：指定了不同的文件类型该如何编译。一般来说，开发者并不需要修改这里面的内容。如果你需要对特定类型的文件添加处理方法，那么可以在此处添加一条新的规则。
